...
/*   Updated: 2024/06/30 17:52:07 by mbartos          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
uint32_t	get_pixel_color(mlx_texture_t *texture, int y_height, int x_width)
{
	...
	color = get_rgba(texture->pixels[i],
...
mlx_texture_t	*get_texture(t_game *game, double h_distance, double v_distance)
{
	...
	{
		if (deg_to_rad(ra) > deg_to_rad(EAST_D)
			&& deg_to_rad(ra) < deg_to_rad(WEST_D))
	...
	{
		if ((deg_to_rad(ra) > deg_to_rad(NORTH_D)
				&& deg_to_rad(ra) < deg_to_rad(SOUTH_D)))
